...
app.layout = html.Div([
			html.H3('Negative reviews'),
			dcc.Graph(id='live_negative',animate=True),
			dcc.Interval(id='graph_update',interval=1000)
		]
	)

@app.callback(Output('live_negative','figure'),
...
def live_update_positive():
	try:
		...
		c = conn.cursor()
		df = pd.read_sql("SELECT * FROM allNegative WHERE subjectivity < 0.5", conn)
